...
export default function ProductDetailPage() {
  ...
  const translatedProduct = getTranslatedProduct(params.slug as string, 'nl');
  ...
  const productSEO = product ? {
    ...
    image: product.image,
    category: getTranslatedCategory(product.category, 'nl'),
  ...
  const breadcrumbData = product ? generateBreadcrumbStructuredData([
    ...
    { name: product.name, url: `http://localhost:3000/nl/products/${product.slug}` }
  ]) : null;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <motion.div className="text-center">
          ...
          <h2 className="text-2xl font-bold text-gray-900 mb-2">Product Niet Gevonden</h2>
          <p className="text-gray-600 mb-4">Het product dat u zoekt bestaat niet.</p>
          <button
            onClick={() => router.push('/nl/products')}
  ...
  return (
    <>
      ...
      <HreflangTagsServer pathname={`/nl/products/${product.slug}`} currentLocale="nl" />
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <motion.nav className="mb-8">
            <ol className="flex items-center space-x-2 text-sm text-gray-600">
              <li>
                <button onClick={() => router.push('/nl')} className="hover:text-yellow-600 transition-colors">
                  Home
                </button>
              </li>
              <li>/</li>
              <li>
                <button onClick={() => router.push('/nl/products')} className="hover:text-yellow-600 transition-colors">
                  Producten
                </button>
              </li>
              <li>/</li>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
              ...
              <div className="bg-white rounded-2xl shadow-xl p-8">
                <div className="relative h-96 mb-6 bg-gradient-to-br from-yellow-50 to-orange-50 rounded-xl overflow-hidden">
                  <Image
                    ...
                    alt={`${product.name} - Premium Poolse honing`}
                    fill
                    ...
                    return (
                      ...
                      >
                        <div className="w-full h-full bg-gradient-to-br from-yellow-50 to-orange-50 flex items-center justify-center">
                          <Image
                            src={imageSrc}
                            alt={`${product.name} - ${index === 1 ? 'Hoofdbeeld' : 'Alternatief beeld'}`}
                            width={80}
                            height={80}
            ...
            <motion.div className="space-y-8">
                          <div className="text-yellow-600 font-medium">{getTranslatedCategory(product.category, 'nl')}</div>
            ...
            <div className="inline-block bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-sm font-medium mb-4">
              {getTranslatedCategory(product.category, 'nl')}
            </div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <span className="text-gray-600">({product.reviews} beoordelingen)</span>
              </div>
              ...
              <div className="space-y-3">
                <h3 className="font-semibold text-gray-900">Belangrijkste Kenmerken:</h3>
              ...
              {/* Quantity Selector */}
              <div className="flex items-center space-x-4">
                <label className="font-medium text-gray-900">Aantal:</label>
           ...
           <motion.section className="mb-16">
             <div className="bg-white rounded-2xl shadow-xl p-8">
                               <h2 className="text-3xl font-bold text-gray-900 mb-8">Klantenbeoordelingen</h2>
          ...
          <motion.section className="mb-16">
            <div className="bg-white rounded-2xl shadow-xl p-8">
              <h2 className="text-2xl font-bold text-gray-900 mb-6">Ontdek Meer</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <motion.div className="text-center p-6 border border-gray-200 rounded-xl hover:border-yellow-300 transition-colors">
                  <h3 className="font-semibold text-gray-900 mb-2">Alle Honingproducten</h3>
                  <p className="text-gray-600 text-sm mb-4">Bekijk onze complete collectie premium Poolse honing</p>
                  <button
                    onClick={() => router.push('/nl/products')}
                ...
                <motion.div className="text-center p-6 border border-gray-200 rounded-xl hover:border-yellow-300 transition-colors">
                  <h3 className="font-semibold text-gray-900 mb-2">Honing Voordelen</h3>
                  <p className="text-gray-600 text-sm mb-4">Leer over honing gezondheidsvoordelen en recepten</p>
                  <button
                ...
                <motion.div className="text-center p-6 border border-gray-200 rounded-xl hover:border-yellow-300 transition-colors">
                  <h3 className="font-semibold text-gray-900 mb-2">Neem Contact Op</h3>
                  <p className="text-gray-600 text-sm mb-4">Neem contact op met onze honingexperts</p>
                  <button
                    onClick={() => router.push('/nl/contact')}
                    className="text-yellow-600 hover:text-yellow-700 font-medium text-sm"
                  >
                    Neem Contact Op →
                  </button>
          ...
          <motion.section>
            <h2 className="text-3xl font-bold text-gray-900 mb-8 text-center">U Vindt Misschien Ook Leuk</h2>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                             {relatedProducts.map((relatedProduct, index) => (
                 <motion.div
                   key={`${relatedProduct.id}-nl`}
                   initial={{ opacity: 0, y: 50 }}
                   animate={{ opacity: 1, y: 0 }}
                   transition={{ delay: index * 0.1 }}
                   whileHover={{ y: -10 }}
                 >
                   <ProductCard product={relatedProduct} locale="nl" />
                 </motion.div>
               ))}
            </div>
          </motion.section>
