...
export default function CheckoutPage() {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-gray-900 mb-4 font-heading">{getCartTranslation('nl', 'cartEmptyTitle')}</h1>
          <p className="text-gray-600 mb-8 font-body">{getCartTranslation('nl', 'cartEmptyMessage')}</p>
          <Link
            href="/nl"
          ...
          >
            {getCartTranslation('nl', 'continueShopping')}
          </Link>
        </div>
      </div>
    );
  ...
  const handlePaymentSuccess = async (paymentIntentId: string) => {
    ...
    try {
      ...
      router.push(`/nl/confirmation?payment_intent=${paymentIntentId}`);
    } catch (error) {
      console.error('❌ Error completing order:', error);
      setError('Er is een fout opgetreden bij het voltooien van uw bestelling. Probeer het opnieuw.');
    }
  };
    ...
    <div className="min-h-screen bg-gray-50 py-12">
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2 font-heading">{getCartTranslation('nl', 'checkout')}</h1>
          <p className="text-gray-600 font-body">{getCartTranslation('nl', 'completeOrderSecurely')}</p>
        </div>
        ...
        <div className="max-w-md mx-auto mb-8">
          <div className="flex items-center justify-between">
            <div className={`flex items-center ${step === 'customer' ? 'text-yellow-600' : 'text-gray-400'}`}>
              ...
              <span className="ml-2 font-medium font-body">{getCartTranslation('nl', 'customerInfo')}</span>
            </div>
            ...
            <div className={`flex items-center ${step === 'payment' ? 'text-yellow-600' : 'text-gray-400'}`}>
              ...
              <span className="ml-2 font-medium font-body">{getCartTranslation('nl', 'payment')}</span>
            </div>
          </div>
        </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-lg p-6 sticky top-8">
                <h3 className="text-lg font-bold text-gray-900 mb-4 flex items-center font-heading">
                  ...
                  {getCartTranslation('nl', 'orderSummary')}
                </h3>
                ...
                <div className="space-y-3 mb-6">
                  {state.items.map((item) => {
                    ...
                    const translatedProduct = getTranslatedProduct(item.product.slug, 'nl');
                      ...
                      <div key={item.product.id} className="flex items-center justify-between py-2 border-b border-gray-100">
                        <div className="flex items-center space-x-3">
                          <div>
                            ...
                            <p className="text-sm text-gray-500 font-body">{getCartTranslation('nl', 'quantity')}: {item.quantity}</p>
                          </div>
                        </div>
                ...
                <div className="space-y-2 border-t border-gray-200 pt-4">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 font-body">{getCartTranslation('nl', 'subtotal')}</span>
                    <span className="font-medium">€{subtotal.toFixed(2)}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600 font-body">{getCartTranslation('nl', 'shipping')}</span>
                    <span className="font-medium">{shipping === 0 ? getCartTranslation('nl', 'free') : `€${shipping.toFixed(2)}`}</span>
                  </div>
                  {loyaltyDiscount > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600 font-body">{getCartTranslation('nl', 'loyaltyDiscount')}</span>
                      <span className="font-medium text-green-600">-€{loyaltyDiscount.toFixed(2)}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-lg font-bold border-t border-gray-200 pt-2">
                    <span className="font-heading">{getCartTranslation('nl', 'total')}</span>
                ...
                {subtotal < 69 && (
                  <div className="mt-4 p-3 bg-yellow-50 rounded-lg">
                    <p className="text-sm text-yellow-800 font-body">
                      {getCartTranslation('nl', 'addForFreeShipping').replace('{amount}', (69 - subtotal).toFixed(2))}
                    </p>
                  </div>
                )}
              </div>
